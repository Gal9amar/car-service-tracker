...
import { sendEmail, buildNewServiceEmailHtml } from '../services/emailService.js';
...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (vehicleId) {
      const vehicle = await prisma.vehicle.findFirst({ where: { id: vehicleId, userId: req.user.id } });
...
router.post('/', async (req, res, next) => {
  try {
    ...
    if (data.mileage && (!vehicle.currentMileage || data.mileage > vehicle.currentMileage)) {
      await prisma.vehicle.update({ where: { id: data.vehicleId }, data: { currentMileage: data.mileage } });
    }

    // ── 2. טיפול תקופתי: סמן תזכורת קודמת כבוצעה + צור תזכורת חדשה ───────
    if (data.serviceType === 'PERIODIC' && data.nextServiceMileage) {
      // סמן תזכורות PERIODIC קודמות כלא פעילות
      await prisma.reminder.updateMany({
        where: {
          vehicleId:    data.vehicleId,
          reminderType: 'CUSTOM',
          title:        { contains: 'טיפול תקופתי' },
          isActive:     true,
        },
        data: { isActive: false },
      });

      // צור תזכורת חדשה — תאריך יעד = תאריך הטיפול + שנה
      ...
      await prisma.reminder.create({
        data: {
          ...
          title:        'טיפול תקופתי',
          dueDate:      nextDueDate,
          dueMileage:   data.nextServiceMileage,
          intervalMonths: 12,
        },
      });
    }
    ...
    try {
      if (vehicle.user?.email) {
        const html = buildNewServiceEmailHtml({
        ...
        await sendEmail({
          ...
          subject: `🔧 טיפול חדש נרשם: ${vehicle.manufacturer} ${vehicle.model}`,
          html,
        });
      }
    } catch (emailErr) {
      console.error('Failed to send service email:', emailErr.message);
    }
