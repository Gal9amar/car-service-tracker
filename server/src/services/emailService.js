...
export function buildNewVehicleEmailHtml({ userName, vehicle }) {
  const v = vehicle;
  const yn = (b) => b ? '✓ כן' : null;
  const rows = [
    ['מספר רכב',          v.licensePlate],
    ['יצרן',              v.manufacturer],
    ['דגם',               v.model],
    ['שנת ייצור',         v.year],
    v.trim         && ['גימור',           v.trim],
    ...
    v.engineModel  && ['מנוע',            v.engineModel],
    v.engineCC     && ['נפח מנוע',        `${v.engineCC} סמ"ק`],
    v.horsePower   && ['כוח סוס',         `${v.horsePower} כ"ס`],
    v.transmission && ['תיבת הילוכים',    v.transmission],
    v.driveType    && ['הנעה',            v.driveType],
    v.bodyType     && ['סוג מרכב',        v.bodyType],
    v.doors        && ['דלתות',           v.doors],
    v.seats        && ['מושבים',          v.seats],
    v.weight       && ['משקל',            `${v.weight} ק"ג`],
    v.frontTire    && ['צמיג קדמי',       v.frontTire],
    v.rearTire     && ['צמיג אחורי',      v.rearTire],
    v.airbags      && ['כריות אוויר',     v.airbags],
    v.towingWithBrakes && ['גרירה עם בלמים', `${v.towingWithBrakes} ק"ג`],
    v.towingWithoutBrakes && ['גרירה ללא בלמים', `${v.towingWithoutBrakes} ק"ג`],
    v.vin          && ['מספר שלדה',       v.vin],
    v.ownership    && ['בעלות',           v.ownership],
    v.origin       && ['מקור',            v.origin],
    v.standardType && ['סטנדרט',          v.standardType],
    v.greenScore   && ['ציון ירוק',       v.greenScore],
    v.pollutionLevel && ['רמת זיהום',     v.pollutionLevel],
    v.co2          && ['CO2',             `${v.co2} גר/ק"מ`],
    v.testExpiry   && ['תוקף רישוי',      new Date(v.testExpiry).toLocaleDateString('he-IL', { day:'numeric', month:'long', year:'numeric' })],
    v.currentMileage && ['קילומטראז\'',  `${Number(v.currentMileage).toLocaleString('he-IL')} ק"מ`],
    v.hasABS           && ['ABS',                 yn(v.hasABS)],
    v.hasStabControl   && ['יציבות אקטיבית',      yn(v.hasStabControl)],
    v.hasPowerSteering && ['הגה כוח',             yn(v.hasPowerSteering)],
    v.hasAC            && ['מיזוג אוויר',          yn(v.hasAC)],
    v.hasRearCamera    && ['מצלמת אחור',           yn(v.hasRearCamera)],
    v.hasAutoEmergencyBrake && ['בלימת חירום',     yn(v.hasAutoEmergencyBrake)],
    v.hasLaneDeparture && ['התראת יציאת נתיב',    yn(v.hasLaneDeparture)],
    v.hasAdaptiveCruise && ['שיוט אדפטיבי',       yn(v.hasAdaptiveCruise)],
    v.hasActiveRecall  && ['⚠️ ריקול',            'יש קריאות שירות פתוחות — בדוק בפרטי הרכב'],
  ].filter(Boolean);
